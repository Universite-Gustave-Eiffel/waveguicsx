...
ws.waveguide0.compute_poynting_normalization() #!!!!!!important!!!!!!!->add a comment
...
mode = ws.waveguide0.track_mode(0, 1, threshold=0.98, plot=True)
ws.set_ingoing_mode('waveguide0', mode) #set a single ingoing mode coeff, to 1 i.e. energy to 1 because poynting normalization!!!!
ws.set_parameters()
...
sc = ws.waveguide0.plot(y=('coefficient', lambda x:np.square(np.abs(x))), direction=-1) #energy!!!!!
sc.axes.set_ylim(0, 1.2)
sc.axes.set_ylabel('energy???') #!!!reflection coefficient (in energy)
#ws.waveguide0.plot_energy_velocity(c=('coefficient', np.abs))
if not free_end:
    sc = ws.waveguide1.plot(y=('coefficient', lambda x:np.square(np.abs(x))), direction=+1)
    sc.axes.set_ylim(0, 1.2)
    sc.axes.set_ylabel('energy')
    #ws.waveguide1.plot_energy_velocity(c=('coefficient', np.abs))

###############################################################################
# Example of plot for a single mode

#modif a faire dans plot de waveguide.py: si mode est donne, alors direction doit etre None (sinon error)!
mode_ref = ws.waveguide0.track_mode(0, 0, threshold=0.98, plot=True)
sc = ws.waveguide0.plot(y=('coefficient', lambda x:np.square(np.abs(x))), mode=mode_ref)
#sc[0].set_marker('')
plt.show()

##################################
# Plotting the displacement field for a given angular frequency
