...
class Scattering:
    ...
    def solve(self):
        ...
        for i, omega in enumerate(self.omega):
            ...
            #ksp.setOperators(Bu_out_transpose*(D*Bu_out-Bf_out)) #the operator A of system Ax=b
            #ksp.setFromOptions()
            #ksp.solve(Bu_out_transpose*(Bf_in-D*Bu_in)*U_in, U_out) #the right-hand side b and the solution x of system Ax=b
    ...
    def plot_energy_balance(self):
        """Plot energy balance at each frequency (for checking modal tbc truncature)"""
        ...
        ax.plot(np.abs(energy_balance[1::3]), linewidth=1, color='red', label='|tbc flux|')
        ax.plot(np.abs(energy_balance[2::3]), linewidth=1, color='blue', label='|volume power|')
        ax.set_xlabel('angular frequency index')
        ax.set_ylabel('power')
        ax.legend()
        fig.tight_layout()
